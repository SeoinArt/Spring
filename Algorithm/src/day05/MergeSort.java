...
//		int arr[] = { 6, 5, 13, 7, 1, 9, 8, 15, 2 };
	...
	static void partition(int[] arr, int left, int right) {
		if (left == right)
			return;// 분할 하다보면 (2로 나누기) left==right순간이 생긴다
	...
	static void merge(int[] arr, int left, int right) {
		...
		if (pl > pc) {
			// 왼쪽 배열을 tmp로 다 옮겼다면 오른쪽 배열의 남은 요소를 tmp로 옮긴다.
			for (int i = pr; i <= right; i++) {
				tmp[index++] = arr[i];
			}

		} else {
			// 반대의 경우, 왼쪽 배열의 남은 요소를 tmp로 옮긴다.
			for (int i = pl; i <= pc; i++) {
				tmp[index++] = arr[i];
			}
		}
		// 임시 배열 tmp를 원본 배열 arr에 카피한다.
