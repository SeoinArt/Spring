...
public class MemoMapperImpl implements MemoMapper {
	...
	public int insertMemo(MemoVO memo) {
		// 먼저 글번호 no에 넣어줄 값을 sequence collection에서 가져오기(증가시켜서 가져오기)
		Update update = new Update().inc("count",1); // count 값을 1씩 증가시킨다. {$inc:{count:1}}
		SeqVO svo = mongoTemplate.findAndModify(query(where("collectionName").is("memo")), update, SeqVO.class);
		log.info("svo==="+svo);
		
		memo.setNo(svo.getCount()); // seq에 해당하는 count 값을 글번호로 설정하자
	...
	public int deleteMemo(String id) {
		DeleteResult drs = mongoTemplate.remove(query(where("_id").is(id)), MemoVO.class);
		return (int)drs.getDeletedCount();
	}
	...
	public int updateMemo(MemoVO memo) {
		 ...
		 * memo.getMsg()); UpdateResult res =
		 * mongoTemplate.updateFirst(query(where("_id").is(memo.getId())),
		 * update,MemoVO.class);
		 */
		memo.setWdate(CommonUtil.getDateTime());
	...
	public MemoVO getMemo(String id) {
		//MemoVO vo = mongoTemplate.findById(id, MemoVO.class);
		MemoVO vo = mongoTemplate.findOne(query(where("_id").is(id)), MemoVO.class);
		return vo;
	}
