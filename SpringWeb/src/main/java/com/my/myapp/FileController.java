...
public class FileController {
	...
	@ResponseBody
	public ResponseEntity<org.springframework.core.io.Resource> fileDownload(@RequestParam("fname") String fname, 
		...
		// 2. FileSystemResource객체 생성해서 파일의 절대 경로 정보를 넘긴다 ==> 알아서 파일과 스트림 연결해서 다운로드 함
		// 2_2. 브라우저별 인코딩 처리
		String downName="";
		boolean checkIE=(userAgent.indexOf("MSIE")>-1||userAgent.indexOf("Trident")>-1); // ie인 경우 TRUE 값 할당
		try {
		if(checkIE) {
			//IE인 경우
			...
			//그 외 브라우저인 경우
			origin_fname=origin_fname.replace(",", "");//크롬은 파일명에 콤마가 있으면 다운로드 되지 않는다.
